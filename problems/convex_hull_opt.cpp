...
struct Solution {
    Solution(int) {
        ...
        vector<Line> stk = {{-2LL * h[1], h[1] * h[1]}};

        auto cmp = [&stk](int i, LL x) {
            return stk[i].intersect_x(stk[i + 1]) < x;
        };
        ...
        for(int i : range(2, N + 1)) {
            ...
            int m = stk.size();
            while(m >= 2 and cur.intersect_x(stk[m - 1]) <= stk[m - 1].intersect_x(stk[m - 2])) {
                stk.pop_back();
                --m;
            }
            stk.push_back(cur);
