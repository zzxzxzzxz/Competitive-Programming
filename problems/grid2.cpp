...
struct Solution {
    Solution(int) {
        ...
        auto calc_ways = [&](auto a, auto b) {
            int h = b.r - a.r;
            int w = b.c - a.c;
            return fac[h + w] * ifac[h] % MOD * ifac[w] % MOD;
        };

        vector<int> dp(N + 1);
        for(int i : range(N + 1)) {
            dp[i] = calc_ways(Block{1, 1}, block[i]);
        }

        for(int i : range(N + 1)) {
            for(int j: range(i)) {
                if(block[j].r <= block[i].r and block[j].c <= block[i].c) {
                    dp[i] += MOD - dp[j] * calc_ways(block[j], block[i]) % MOD;
