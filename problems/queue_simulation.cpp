...
struct Solution {
    Solution(int) {
        ...
        priority_queue<PLI, vector<PLI>, greater<PLI>> events;
        deque<int> que;
        ...
        vector<int> t(n), tidx(n);
        for(int i : range(n)) {
            read(t[i]);
            events.push({t[i], 0});
        }

        iota(tidx.begin(), tidx.end(), 0);
        sort(tidx.begin(), tidx.end(), [&](int i, int j) { return pair{t[i], i} < pair{t[j], j}; });
        int j = 0;
        ...
        while(events.size()) {
            ...
            if(ready.size() and (que.empty() or que.back() > ready.top())) {
                if(que.empty()) {
                    events.push({T + p, 1});
                }
                que.push_back(ready.top());
                ready.pop();
            }
        }

        for(auto i : printer(ans)) {
            print_1(i);
        }
    }
};
