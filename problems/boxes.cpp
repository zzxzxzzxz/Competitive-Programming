...
struct Solution {
    Solution(int) {
        ...
        int mx = 0;
        for(int i : range(N)) {
            setmax(mx, boxes[i].s + boxes[i].w);
        }

        sort(boxes.begin(), boxes.end(),
            [](auto& a, auto& b) { return a.s + a.w < b.s + b.w; });

        vector<LL> dp(mx + 1, -1);
        dp[0] = 0;
        LL best = 0;
        for(int i : range(N)) {
            for(int j : range(boxes[i].s + boxes[i].w, boxes[i].w - 1, -1)) {
                if(dp[j - boxes[i].w] != -1) {
                    setmax(dp[j], dp[j - boxes[i].w] + boxes[i].v);
                    setmax(best, dp[j]);
                }
            }
        }
