...
struct Solution {
    int R, C;
    int dfs(int r, int c, int r0, int r1, int c0, int c1, int colors) {

        int cid = (not (r >= r0 and r <= r1)) * 2 + (not (c >= c0 and c <= c1));
        if(color[r][c] != (colors >> cid & 1) or visited[r][c]) {
            return 0;
        }
        visited[r][c] = true;
        ...
        for(auto& dir : dirs) {
            ...
            ans += dfs(r2, c2, r0, r1, c0, c1, colors);
        }
        return ans;
    }
    ...
    Solution(int) {
        ...
        for(int i : range(R)) {
            for(int j : range(C)) {
                color[i][j] = (dream[i][j] == 'B');
            }
        }

        auto encode = [](int a, int b, int c, int d) {
            return a + (b << 1) + (c << 2) + (d << 3);
        };

        array<bool, 16> pattern = {};
        for(int i : range(R)) {
            for(int j : range(C)) {
                {
                    pattern[encode(color[i][j], color[i][j], color[i][j], color[i][j])] = true;
                }
                if(i < R - 1) {
                    pattern[encode(color[i][j], color[i][j], color[i + 1][j], color[i + 1][j])] = true;
                }
                if(j < C - 1) {
                    pattern[encode(color[i][j], color[i][j + 1], color[i][j], color[i][j + 1])] = true;
                }
                if(i < R - 1 and j < C - 1) {
                    pattern[encode(color[i][j], color[i][j + 1], color[i + 1][j], color[i + 1][j + 1])] = true;
                }
            }
        }
        ...
        for(int i : range(R)) {
            for(int j : range(C)) {
                for(int colors = 0; colors < (1 << 4); ++colors) {
                    if(not pattern[colors]) continue;

                    memset(visited, 0, sizeof(visited));
                    int res = 0;
                    for(int r = 0; r < R; ++r) {
                        for(int c = 0; c < C; ++c) {
                            setmax(res, dfs(r, c, 0, i, 0, j, colors));
                        }
                    }
                    setmax(ans, res);
                }
